...
int placePion_IA(int tab[24], int turn[2], int* dm)//Place un pion sur le tableau
{
	*dm = rand() % 23;
	if (tab[*dm] == 0)
	{
...
void phase1_IA(int tab[24], int turn[2], int* dm, int pions[2]) //phase 1 s'arrêtant quand les pions des 2 joueurs ont tous étés placés
{
	for (int i = 0; i < 18; i++)
	{
		if (turn[0] == 1)
		...
		{
			*dm = placePion_IA(tab, turn, dm);
			affPlateau(tab);
			cout << "L'IA joue sur la case " << *dm << endl;
			if (check_moulin(tab, turn, dm, 0))
			{
				supprPion(tab, turn, pions); affPlateau(tab);
			}
...
void unjoueur(int tab[24], int turn[2], int* dm, int pions[2])
{
	affPlateau(tab);
	phase1_IA(tab, turn, dm, pions);
}

void deuxjoueurs(int tab[24], int turn[2], int* dm, int pions[2])
{
	affPlateau(tab);
	phase1(tab, turn, dm, pions);
	phase2(tab, turn, dm, pions);
}
...
void jeu()
{
	...
	ClearScreen();
	int nbrJoueurs;
	cout << "Entrez le nombre de joueur(s) (1 ou 2): ";
	cin >> nbrJoueurs;
	if (nbrJoueurs == 2)
	{
		deuxjoueurs(tableau, turn, &dermove, pions);
	}
	else
	{
		unjoueur(tableau, turn, &dermove, pions);
	}
	string choix;
	cout << "si vous voulez rejouer une partie entrez 'y' " << endl;
	cout << "si vous voulez vous arreter entrez 'n' " << endl;
	cout << "votre choix: ";
	cin >> choix;
	if (choix == "y")
	{
